function Pr = log_normal_shadowing(scenario,hut)
%% start extra code 
...
%%%%%%%%% calculating lognormal shadowing for diffrent channel models of
%%%%%%%%% UAV
switch(scenario)
     case 'RMa'
...
rstate = randn('state');
randn('state', d);
powerLoss_shadowing_db = (randn*std_db+0);  % random('Normal', 0, std_db);
randn('state', rstate);

% calculate the receiving power at distance d
Pr = powerLoss_shadowing_db ;
