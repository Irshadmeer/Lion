...
%   
%   for k = 1: length(serialGBSAA)
%    
%       if (serialGBSAA(k)>firstpacketarrival && serialGBSAA(k)< (firstpacketarrival+0.000120) )
%         
...
% 
% 
% percentage(q,1:4) = [percentage1,percentage2,percentage3,percentage4];
% X = [x1,x2,x3,x4];
% end
% 
% %percentage
...
randomArrival55 = (bb-aa).*rand(5,1) + aa;
...
randomArrival555 = (bbb-aaa).*rand(5,1) + aaa;
...
xlabel('No of Incoming helicopter'); 
ylabel('Collision Prob. (percentage)'); 
title('GBSAAsorted Collision Probability'); 
